...
// BUS CONNECTIVITY
uint8_t j6502::read(uint16_t a) {
	return bus -> read(a, false);
}

void j6502::write(uint16_t a, uint8_t d) {
	bus -> write(a, d);
}
...
void j6502::clock()
{
	if (cycles == 0)
	{
		...
		SetFlag(j6502::U, true);

#ifdef LOGMODE
		if (logfile == nullptr)	logfile = fopen("j6502.txt", "wt");
		if (logfile != nullptr)
		{
...
// FLAGS
// Return The Value Of Specific Bit Of Stats Reg
uint8_t j6502::GetFlag(FLAGS6502 f) {
	return ((status & f) > 0) ? 1 : 0;
}

// Sets/Clears a Specific Bit Of Status Reg
void j6502::SetFlag(FLAGS6502 f, bool v) {
	if (v) {
		status |= f;
	}
	else {
...
// 3 Address Mode Use Indirection [aka Pointers]
// Mode: Indirect
uint8_t j6502::IND() {
...
std::map<uint16_t, std::string> j6502::disassemble(uint16_t nStart, uint16_t nStop)
{
